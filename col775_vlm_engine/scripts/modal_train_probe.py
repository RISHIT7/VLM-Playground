...
def train_probe_remote(args: list[str]):
    download_dataset()
    ...
    # By default, the linear probe looks for CLIP and DINO checkpoints inside checkpoints/.
    # Provide helpful default defaults if they start with checkpoints/ from local,
    # but let user override.

    print(f"Running Linear Probe training with args: {cli_args}")
    sys.argv = cli_args
...
def main(*args: str):
    ...
    e.g. `modal run scripts/modal_train_probe.py -- --probe-clip-ckpt /checkpoints/clip/...pt`
    """
    train_probe_remote.remote(list(args))
